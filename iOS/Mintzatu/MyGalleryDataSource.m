...
@implementation MyGalleryDataSource

- (id)initWithCompletitonBlock:(void (^)(BOOL success))block
{
    self = [super init];
    if (self) {
...
- (void)loadData
{
    if (_finishLoading == NO) {
        ...
        [[MintzatuAPIClient sharedClient] postPath:@"my-pictures" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            for (int i = 0; i < 20 && i < picturesData.count; i++) {
                ...
                NSArray *array = [NSArray arrayWithObjects:[pictureDict objectForKey:@"imgUrl"], [pictureDict objectForKey:@"imgUrl"], nil];
