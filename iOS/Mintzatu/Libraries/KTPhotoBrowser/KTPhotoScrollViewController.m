...
- (void)dealloc 
{
   ...
   //[_photoDescriptionView release], _photoDescriptionView = nil;
...
- (void)loadView 
{
   ...
   [space release];
    ...
    if (_showDetailView) {
        NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"PhotoDescriptionView" owner:nil options:nil];
        _photoDescriptionView = (PhotoDescriptionView*) [nib objectAtIndex:0];
        CGRect photoDescriptionViewFrame = _photoDescriptionView.frame;
        photoDescriptionViewFrame.origin.y = self.view.frame.size.height - photoDescriptionViewFrame.size.height;
        _photoDescriptionView.frame = photoDescriptionViewFrame;
        [self.view addSubview:_photoDescriptionView];
    }
}
...
{
    NSDictionary *photoDescription = [dataSource_ getPhotoDescriptionAtIndex:currentIndex_];
    _photoDescriptionView.nameLabel.text = [photoDescription objectForKey:@"username"];
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:[[photoDescription objectForKey:@"date"] floatValue]];
    _photoDescriptionView.dateLabel.text = [_dateFormatter stringFromDate:[date toLocalTime]];
}
